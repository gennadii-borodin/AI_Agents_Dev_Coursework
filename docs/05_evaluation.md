...
### 1.1 Успех
Фиксируется в корневом спане прогона (`src/report.py:314-396`):
...
- Per-run latency фиксируется в `trace_run` (`src/tracing.py:227`) через `duration_ms` на каждом спане.
- Eval harness замеряет `latency_s` каждого кейса (`eval/run_eval.py:29-31`) и агрегирует min/max/mean.

### 1.3 Стоимость (token $)
В `run_review` (`src/report.py:373-386`) на основе `get_run_stats()` (накопленные токены по моделям) и `settings.model_pricing`:
...
|---|---|---|
| Retry LLM-вызовов | `@retry(stop_after_attempt(llm_retry_attempts=3), wait_exponential)` на `chat_completion`/`_do_request` | `src/llm_provider.py:71`, `:132` |
| Retry эмбеддингов | `@retry(stop_after_attempt(3), wait_exponential)` на `_embed_uncached` | `src/embedding.py:19` |
| Retry агентов (targeted) | `quality_gate`: повтор ONLY упавших агентов через `Command(goto=Send)` | `src/graph.py:329-336` |
| Лимит retry-циклов | `max_retry_attempts=2` + `state.iteration` (защита от петли) | `src/config.py:50`, `src/graph.py:325` |
| Лимит чанков Standards | `standards_max_iterations=200` (hard cap runaway-цикла) | `src/agents/standards_agent.py:221` |
| Лимит строк SQL | `sql_max_rows=1000` (truncate + флаг) | `src/tools/sql_tool.py:52` |
| Лимит tool-итераций | `max_iterations` в `invoke_with_tools` | `src/llm_provider.py:190` |
| Circuit-breaker (по сути) | при исчерпании retries LLM — агент не падает графом, а помечается `partial`/`error` и исключается из знаменателя compliance | `src/agents/standards_agent.py:232-237` |
...
- **Structured output:** запрос через `response_format={"type":"json_schema", ...}` (`build_json_schema`, `src/prompts.py:68`); фоллбэк на `json_object` при неподдерживаемой схеме (`src/llm_provider.py:108`).
...
- `StandardsReport.compliance_percentage` считается в коде: `passed_checks / (analyzed_count * num_rules)` (`src/agents/standards_agent.py:247`), знаменатель — число активных правил из YAML (не магическая константа).

### 4.3 Блокировка некорректных действий (SQL/данные)
- **Forbidden keywords:** `execute_sql` отсекает `INSERT/UPDATE/DELETE/DROP/CREATE/ALTER/TRUNCATE` до выполнения (`src/tools/sql_tool.py:36`).
- **Только SELECT:** проекция колонок без вектора исключает утечку тяжёлых payload.
- **Валидация аргументов tool:** `ToolRegistry._validate` (`src/skills.py:87`) — обязательные параметры, приведение `int`.
...
- Реестр `data/standards_rules.yaml` размечает `blocking`/`auto_fixable` правила; `blocking_violations` выделяются отдельно (`src/agents/standards_agent.py:259`), что даёт чёткий критерий «не релизить».
