...
- **Coverage Agent** — семантический поиск ТК, косвенно покрывающих требования (см. раздел 3).
- Вспомогательные хелперы: `rag_search_by_requirement` (точный SQL по `req`), `rag_search_related_requirements` (семантика по `requirements`).
